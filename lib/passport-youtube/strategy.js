...
function Strategy(options, verify) {
  ...
  this.name = 'youtube';
  this._profileURL = options.profileURL || 'https://www.googleapis.com/youtube/v3/channels?part=id%2Csnippet&mine=true';
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.getProtectedResource(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      var youtubeChannel = json.items[0] || {};

      var profile = { provider: 'youtube' };

      profile.id = youtubeChannel.id;
      profile.username = youtubeChannel.snippet.title;
      profile.displayName = youtubeChannel.snippet.title;
